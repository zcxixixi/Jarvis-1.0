    ...
    try:
        if platform.system() == "Darwin":
            ...
            if sound_file.endswith(".wav"):
                ...
                subprocess.Popen(["mpg123", "-o", "pulse", "-q", sound_file],
                               stdout=subprocess.DEVNULL,
                               stderr=subprocess.DEVNULL)
            else:
                subprocess.Popen(["pw-play", sound_file],
...
def play_boot_sound():
    ...
    try:
        if platform.system() == "Darwin":
            ...
            # Use mpv for mp4 audio (pw-play doesn't support mp4 containers)
            subprocess.Popen(["mpv", "--ao=pulse", "--no-video", "--really-quiet", boot_file],
                           stdout=subprocess.DEVNULL,
                           stderr=subprocess.DEVNULL)
    except Exception as e:
