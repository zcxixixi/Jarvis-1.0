...
## 🔌 硬件接线指南 (Hardware Setup)

在通电之前，请先完成硬件连接：

1.  **电源 (Power)**
    *   使用官方电源适配器（USB-C接口）。
    *   Pi 4/5 建议使用 15W+ 的电源，防止供电不足。

2.  **麦克风 (Microphone)**
    *   将 USB 麦克风插入树莓派任意一个 **USB 端口**。
    *   *提示：建议使用 USB 延长线将麦克风稍微远离树莓派本体，减少电流噪声干扰。*

3.  **音箱 (Speaker)**
    *   **方案 A (蓝牙)**：只接电源即可，稍后我们通过软件配对。
    *   **方案 B (AUX 3.5mm)**：如果音箱支持，用音频线连接树莓派的耳机孔（Zero系列除外）。
    *   **方案 C (USB 音箱)**：直接插入 USB 端口。

4.  **显示器/键盘 (初次配置建议)**
    *   如果不用 SSH，建议接上 HDMI 显示器和 USB 键盘进行首次配置。
