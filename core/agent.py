...
class JarvisAgent:
    ...
    async def plan(self, user_input: str) -> ExecutionPlan:
        ...
        # Build planning prompt with available tools
        tool_descriptions = "\n".join([
            f"- {name}: {tool.__class__.__name__}" 
            for name, tool in list(self.tools.items())[:10]  # Show first 10
        ])
        
        planning_prompt = f"""You are a task planner. Given a user request, break it down into executable steps using available tools.
...
Analyze the request and respond with a JSON plan:
{{
  "steps": [
    {{"tool": "tool_name", "description": "what this does", "args": {{"key": "value"}}}},
    ...
  ]
}}

If no tools are needed (simple conversation), return: {{"steps": []}}
...
Response (JSON only):"""
        ...
        try:
            ...
            if matched_tools:
                ...
                if inferred:
                    ...
                    llm_plan = self._plan_with_doubao(user_input, planning_prompt)
    ...
    def _plan_with_doubao(self, user_input: str, system_prompt: str) -> dict:
        """
        Call Doubao (Volcengine) HTTP API for planning
        """
        import requests
        import json
        import os
        ...
        # Note: Volcengine usually needs an Endpoint ID (ep-...) for the precise model
        # We will try to use the access token. If user hasn't set DOUBAO_ENDPOINT_ID, we might fail.
        # But for now we build the structure.
        
        api_key = os.getenv("DOUBAO_ARK_API_KEY") # Updated for LLM Service
        endpoint_id = os.getenv("DOUBAO_ENDPOINT_ID") # User needs to set this!
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"❌ Doubao HTTP Error: {resp.text}")
                return None
        except Exception as e:
    ...
    async def execute_step(self, step: PlanStep) -> Optional[str]:
        ...
        if step.tool_name is None:
            ...
            step.result = "Conversation response (no tool needed)"
            return step.result
